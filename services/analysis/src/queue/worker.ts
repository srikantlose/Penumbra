...
import { createRedisConnection, queueNameForTier, type AnalyzePositionJobData } from './queues.js';
...
async function processJob(db: Database, job: Job<AnalyzePositionJobData>) {
...
async function main() {
  ...
  const workers = TIERS.map(
    (tier) =>
      new Worker<AnalyzePositionJobData>(queueNameForTier(tier), (job) => processJob(db, job), {
        connection,
